        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          ...
          {showDateFilter && (
            <div className="flex flex-col sm:flex-row gap-2">
              <Skeleton className="h-9 w-full sm:w-[150px] md:w-[180px]" />
              <Skeleton className="h-9 w-full sm:w-[150px] md:w-[180px]" />
            </div>
          )}
        ...
        <div className="rounded-md border w-full min-w-full overflow-hidden">
          <div className="bg-muted/50 border-b">
            <div className="flex p-3 gap-2">
              {Array(columnCount).fill(null).map((_, i) => (
                <Skeleton 
                  key={`header-${i}`} 
                  className={`h-5 ${i === 0 ? 'w-6' : i === columnCount - 1 ? 'w-8 ml-auto' : 'w-24 flex-1'} ${i > 3 ? 'hidden lg:block' : i > 2 ? 'hidden md:block' : ''}`} 
                />
              ))}
            </div>
          </div>
          ...
          <div>
            {Array(rowCount).fill(null).map((_, rowIndex) => (
              <div key={`row-${rowIndex}`} className="flex p-3 border-b items-center gap-2">
                {Array(columnCount).fill(null).map((_, colIndex) => (
                  <Skeleton 
                    key={`cell-${rowIndex}-${colIndex}`} 
                    className={`h-4 ${colIndex === 0 ? 'w-4' : colIndex === columnCount - 1 ? 'w-8 ml-auto' : colIndex === columnCount - 2 ? 'w-16' : 'w-24 flex-1'} ${colIndex > 3 ? 'hidden lg:block' : colIndex > 2 ? 'hidden md:block' : ''}`} 
                  />
                ))}
              </div>
            ))}
